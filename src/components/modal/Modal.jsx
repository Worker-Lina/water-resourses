...
const Modal = ({id, active, setActive}) => {
  ...
  return <>
    {loading ? 
      <div className={active ? "modal active" : "modal"}>
        ...
        <p className="text__description"> {item.goal}</p>

        <p className="text__title">Ожидаем результат</p>
        <p className="text__description"> {item.expectation}</p>

        <p className="text__title">Наличие воды в объекте</p>
        <p className="text__description"> {item.water_presence}</p>

        <div className="line"></div>
        <div className="modal__buttons">
            <MyButton variant = "orange"><Link to={"/report-object/"+id}> {t("button")}</Link>
