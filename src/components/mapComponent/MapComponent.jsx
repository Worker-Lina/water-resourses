...
import {  polygones, polylines, titles } from './MapData';
...
const MapComponent = ({textVisible, setTextVisible}) => {
  ...
  const maxBounds = [[42.191498999251266,66.718858623291],[43.81611471713369,71.40727415063473]]
  const mapRef = useRef()
  ...
  function addPolylinesToMap(){
    ...
    for(let i=0;i<polylines.length;i++){
      const antPolyline = antPath(polylines[i].positions,  { use: polyline, delay: 500, color: polylines[i].color, 
      pulseColor: polylines[i].pulseColor,  dashArray: [5,20]});
     ...
     <MapContainer whenCreated={ mapInstance => { mapRef.current = mapInstance; addPolylinesToMap() } } className="myMap" center={center} zoom={zoom} scrollWheelZoom={false} zoomControl={false} doubleClickZoom={false} maxBounds={maxBounds}>
      ...
      markers.map(marker => 
        <Marker key={marker.id} position={marker.location} icon={marker.type.id === 1 ? blueIcon : marker.type.id === 2 ? greenIcon : orangeIcon} title={marker.name} eventHandlers={{click:()=>{
          ...
          if(active){ // если выделенный объект был закрыт, сработает данный код, он вернет карту в исходное положение
            const map = mapRef.current
      ...
      <div className="full-screen" onClick={() => setTextVisible(!textVisible)}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
